import csv
  ...
  for origin, destination, duration in reader: # loop gives each column a name
      db.CS50flights.execute("INSERT INTO flights (origin, destination, duration) VALUES (:origin, :destination, :duration)",
  ...
  db.CS50flights.commit() # transactions are assumed, so close the transaction finished
